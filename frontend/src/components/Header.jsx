...
const Header = () => {
  return (
    <div className='flex flex-col justify-between items-center text-cnter my-20'>
      ...
      <h1 className='text-4xl max-w-[300px] sm:text-7xl sm:max-w-[590px] mx-auto mt-10 text-center drop-shadow-lg'>Turn text to <span className='text-green-500'>image</span>,in seconds</h1>
      ...
      <button className='sm:text-lg text-white bg-black w-auto mt-8 px-12 py-2.5 flex item-center gap-2 rounded-full'>Generate Images</button>
        <div className='flex flex-wrap justify-center mt-16 gap-3'>
          {Array(6).fill('').map((item,index) =>(
            <img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10'
             src="" alt="img" key={index} width={70} />
          ))}
        </div>
